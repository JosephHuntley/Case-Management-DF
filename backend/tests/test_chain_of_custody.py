...
def test_create_chain_of_custody(client_factory, db_session):
    ...
    item = client.post("/api/evidence-items", json={
        ...
        "evidence_tag": f"E-0001-P-ATL {uuid4()}",
    ...
    assert response.json()["from_person"] == None
    assert response.json()["to_person"] == user.json()
...
def test_get_chain_by_id(client_factory, db_session):
    ...
    assert response.json()[0]["from_person"] is None
    assert response.json()[0]["to_person"] == user.json()
